...
export const getOrderUpdateEmail = ({ user, order, status, trackingNumber }) => {
    const templates = {
        ...
        'Confirmation': {
            ...
            html: `
                <div style="font-family: Arial, sans-serif; color: #000; max-width: 600px; margin: auto; border: 1px solid #eee; padding: 20px; border-radius: 12px;">
                    <h1 style="color: #000;">Thanks for your order, ${user.firstName}!</h1>
                    <p>We've received it and will get started on it right away. You'll receive another email once your order has shipped.</p>
                    <p style="font-size: 16px; font-weight: bold;">Order #${order._id.toString().substring(0, 8)}</p>
                    
                    <h2 style="border-bottom: 1px solid #eee; padding-bottom: 10px; margin-top: 30px;">Your Order Summary</h2>
                    ...
                    <p style="margin-top: 30px; text-align: center; font-size: 12px; color: #666;">Thank you for your purchase from CellExpress!</p>
                </div>`
        },
        ...
        'Shipped': {
            ...
            html: `
                <div style="font-family: Arial, sans-serif; color: #000; max-width: 600px; margin: auto; border: 1px solid #eee; padding: 20px; border-radius: 12px;">
                    <h1 style="color: #000;">Get Ready, ${user.firstName}!</h1>
                    ...
                    <table style="width: 100%; border-collapse: collapse; font-size: 14px;">${renderOrderItems(order.orderItems)}</table>
                    <p style="text-align: right; font-size: 18px; font-weight: bold; margin-top: 20px;">Total: LKR ${order.totalPrice.toLocaleString()}</p>
                    <p style="margin-top: 30px; text-align: center; font-size: 12px; color: #666;">Thank you for your purchase from Your Shop Name!</p>
                </div>`
        },
        ...
        'Delivered': {
            ...
            html: `
                <div style="font-family: Arial, sans-serif; color: #000; max-width: 600px; margin: auto; border: 1px solid #eee; padding: 20px; border-radius: 12px;">
                    <h1 style="color: #000;">It's Arrived, ${user.firstName}!</h1>
                    ...
                    <table style="width: 100%; border-collapse: collapse; font-size: 14px;">${renderOrderItems(order.orderItems)}</table>
                    <p style="text-align: right; font-size: 18px; font-weight: bold; margin-top: 20px;">Total: LKR ${order.totalPrice.toLocaleString()}</p>
                    <p style="margin-top: 30px; text-align: center; font-size: 12px; color: #666;">We look forward to seeing you again soon!</p>
                </div>`
        },
        ...
        'Cancelled': {
            ...
            html: `
                <div style="font-family: Arial, sans-serif; color: #000; max-width: 600px; margin: auto; border: 1px solid #eee; padding: 20px; border-radius: 12px;">
                    <h1 style="color: #000;">Order Cancellation Notice</h1>
                    ...
                    <table style="width: 100%; border-collapse: collapse; font-size: 14px;">${renderOrderItems(order.orderItems)}</table>
                    <p style="margin-top: 30px; text-align: center; font-size: 12px; color: #666;">We apologize for any inconvenience this may have caused.</p>
                </div>`
        }
    };
